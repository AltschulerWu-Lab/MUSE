...
def structured_embedding(x, y, z, label_x, label_y, label_z, dim_latent, triplet_margin,
    ...
    Parameters:
        ...
        z:              input batches for the third modality; matrix of n * r, where n = batch size, r is the feature dimension
        label_x:        input sample labels for modality x
        label_y:        input sample labels for modality y
        label_z:        input sample labels for modality z
        dim_latent:          dimension of joint latent representation
    ...
    Outputs:
        ...
        trip_loss_z:    triplet loss for z
    ...
    # encoder
    latent, encode_x, encode_y, encode_z = multiview_encoder(x, y, z, n_hidden, dim_latent)

    # decoder
    w_init = tf.initializers.random_normal()
    w_x = tf.get_variable('w_selection_x', [latent.get_shape()[1], latent.get_shape()[1]], initializer=w_init)
    w_y = tf.get_variable('w_selection_y', [latent.get_shape()[1], latent.get_shape()[1]], initializer=w_init)
    w_z = tf.get_variable('w_selection_z', [latent.get_shape()[1], latent.get_shape()[1]], initializer=w_init)

    with tf.variable_scope("decoder_x"):
        latent_x = tf.matmul(latent, w_x)
        x_hat = decoder(latent_x, n_hidden, x.get_shape()[1])

    with tf.variable_scope("decoder_y"):
        latent_y = tf.matmul(latent, w_y)
        y_hat = decoder(latent_y, n_hidden, y.get_shape()[1])

    with tf.variable_scope("decoder_z"):
        latent_z = tf.matmul(latent, w_z)
        z_hat = decoder(latent_z, n_hidden, z.get_shape()[1])
    ...
    sparse_z = tf.sqrt(tf.reduce_sum(tf.reduce_sum(tf.square(w_z), axis=1)))
    sparse_penalty = sparse_x + sparse_y + sparse_z
    ...
    reconstruct_z = tf.reduce_mean(tf.norm(z_hat - z))
    reconstruct_loss = reconstruct_x + reconstruct_y + reconstruct_z

    # triplet errors
    trip_loss_x = batch_hard_triplet_loss(label_x, latent, triplet_margin)
    trip_loss_y = batch_hard_triplet_loss(label_y, latent, triplet_margin)
    trip_loss_z = batch_hard_triplet_loss(label_z, latent, triplet_margin)

    loss = reconstruct_loss + weight_penalty * sparse_penalty \
           + triplet_lambda * trip_loss_x \
           + triplet_lambda * trip_loss_y \
           + triplet_lambda * trip_loss_z

    return latent, x_hat, y_hat, z_hat, encode_x, encode_y, encode_z, \
           loss, reconstruct_loss, sparse_penalty, trip_loss_x, trip_loss_y, trip_loss_z


def multiview_encoder(x, y, z, n_hidden, dim_latent):
    """
    Encoder combines x, y and z to a joint latent representation
    ...
    with tf.variable_scope("encoder_z"):
        h_z = encoder(z, n_hidden)

    # combine h_x, h_y and h_z to joint latent representation
    w_init = tf.keras.initializers.VarianceScaling()
    b_init = tf.constant_initializer(0.)
    h = tf.concat([h_x, h_y, h_z], 1)
    wo = tf.get_variable('wo', [h.get_shape()[1], dim_latent], initializer=w_init)
    bo = tf.get_variable('bo', [dim_latent], initializer=b_init)
    latent = tf.matmul(h, wo) + bo

    return latent, h_x, h_y, h_z
